...
- DX12: wire `BlurPassDX12` into `NativeRendererDX12::RenderShadowBatch()` (`BatchKind::Shadow`), mirroring Vulkan's own Phase 35.4 — not yet started; must additionally handle the render-target/UAV resource-state transition Vulkan's own equivalent didn't need (see above).
- DX12: `Layer`/`BackdropBlur` parity work, mirroring Vulkan's own Phase 35.5-35.6 — not yet started.

...
- WebGPU: `Shadow`/`Layer`/`BackdropBlur` parity work, mirroring Vulkan's own Phase 35.4-35.6 sequence — `Shadow` (wiring `BlurPassWebGPU` into a silhouette-render-then-blur-then-composite flow) is next; `Layer`/`BackdropBlur` remain after that.

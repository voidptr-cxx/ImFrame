...
- WebGPU: `BackdropBlur` parity, mirroring Vulkan's Phase 35.6/DX12's Phase 35.14 — not yet started. Its region copy can use `wgpuCommandEncoderCopyTextureToTexture()` with an origin/extent, reading `_currentTargetTexture` (now tracked). It must follow the same submit-before-writing-shared-buffers rule as `PopLayer()`.

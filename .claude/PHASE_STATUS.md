...
**Next Action:** Phase 35.2 (complete, this session) added `BatchKind::Image` to `NativeRendererVulkan` — `Rendering::TextureId::Value()` reinterpreted as a raw `VkImageView`, sampled through one shared `VkSampler`, with one fresh per-batch descriptor set allocated from a pool reset every `Render()` call (necessary because a descriptor set's *content* resolves at GPU execution time, not `vkCmdBindDescriptorSets` recording time — reusing one set across differently-textured `Image` batches in the same not-yet-submitted command buffer would make every draw sample whichever texture was written last). **Found and fixed a real, pre-existing Phase 35.1 bug in the process:** `SDFRect.glsl`/`Image.glsl`'s vertex shader negated Y (`gl_Position = vec4(ndc.x, -ndc.y, 0, 1)`), copied verbatim from `NativeRendererGL3`'s own shader where that negation is *correct* (GL's NDC is bottom-up) — but Vulkan's NDC is already top-down, so the same negation was a real, wrong double-flip, invisible in every Phase 35.1 test because they were all flip-symmetric (full-height rects, or samples taken exactly at the vertical center). Phase 35.2's own off-center, non-full-height combined test exposed it; fixed by removing the negation in both shaders. Also fixed an unrelated, real bug the validation layer caught once a Debug (not Release) build was used to get diagnostics: the test file's `ScratchImage` helper was missing `VK_IMAGE_USAGE_TRANSFER_DST_BIT`, needed by its own `vkCmdClearColorImage` call. Full suite **526/526** (524 baseline + 2 new tests), `check_api_leaks.sh` clean, both Debug (validation-clean) and Release rebuilt and passing. See `.claude/DECISIONS.md`'s "Phase 35.2" section for the full reasoning and debugging trail. Committed on `feature/phase-35.2-vulkan-image` as `f3036ff`, merged into `develop` as `ab241a6` (clean, no conflicts).

**Remaining Phase 35 work, roughly in order (not yet broken into named sub-phases beyond 35.2):**
- Vulkan: compute-shader Gaussian blur (`vkCmdDispatch`, transient `VkImage` via VMA per the proposal) + shadow/layers/backdrop-blur mirroring GL3's Phase 34 work, then real integration into `SDL3VulkanBackend`'s live frame loop (replacing the current per-`Render()`-call CPU stall with proper semaphore sync) and `IRenderer` batching/text.

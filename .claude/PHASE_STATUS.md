...
- DX12: `Layer`/`BackdropBlur` parity work, mirroring Vulkan's own Phase 35.5-35.6 — not yet started; `Layer` will need its own D3D12-specific resource-state reasoning for `PushOpacityLayer`/`PushBlendLayer`'s own offscreen targets, following the discipline established in Phase 35.12.

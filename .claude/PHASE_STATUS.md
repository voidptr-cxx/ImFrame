...
**Next Action:** Phase 35.7 (complete, this session) is DX12's own "first SDF rect" sub-phase — a new `NativeRendererDX12` (`Backends/SDL3DX12/`), mirroring `NativeRendererVulkan`'s Phase 35.1 scope and structure exactly (`BatchKind::Rect` only, lazy D3D12 resource init, streaming upload-heap vertex/index/constant buffers, a synchronous per-`Render()`-call fence wait matching `ViewportFramebufferDX12`'s own identical Phase 24 stopgap). New `Shaders/SDFRect.hlsl` (compiled to DXIL via a new `cmake/CompileShaderDXC.cmake`/`dxc.exe` pipeline, mirroring `cmake/CompileShader.cmake`'s own SPIR-V pipeline shape) keeps both `VSMain`/`PSMain` in one file — HLSL supports multiple named entry points natively, so no `STAGE_VERTEX`/`STAGE_FRAGMENT`-style macro guard is needed the way GLSL's own single-file convention requires. **`VSMain` negates Y, matching `NativeRendererGL3`'s own shader, not `NativeRendererVulkan`'s** — D3D12's NDC is Y-up (same convention as GL), unlike Vulkan's Y-down NDC; derived independently here rather than assumed by analogy, and verified via a deliberately off-center asymmetric-placement test written from this sub-phase's *first* commit (directly following Phase 35.2's own stated lesson, not retrofitted after a bug). The PerFrame constant buffer is bound via a root CBV — no descriptor heap needed at all for this sub-phase's scope. **Found and fixed a real, pre-existing, unrelated bug** while getting `IMF_BUILD_DX12_BACKEND=ON` to compile for the first time this session: `ViewportDX12.cpp` silently discarded `DescriptorAllocator::Allocate()`'s own `[[nodiscard]] bool` return value (RTV and SRV slots both) — fixed correctly (call first, store the bool, assert on the local), not by naively wrapping the call directly in `IMF_ASSERT(...)`, which would have made the call itself vanish in Release builds (`IMF_ASSERT`'s own documented contract: its condition must have no side effects, elided entirely under `NDEBUG`). **Landed clean on the first real attempt** — zero D3D12 debug-layer warnings across all 3 new tests. Full suite **553/553** (533 baseline + 18 `[dx12]`-labelled tests — `IMF_BUILD_DX12_BACKEND` newly enabled in this session's own build directory for the first time, so those 18 include pre-existing Phase 22 tests never previously exercised here, plus this sub-phase's own 3 new ones), `check_api_leaks.sh` clean, both Debug (D3D12-debug-layer-clean) and Release rebuilt and passing. See `.claude/DECISIONS.md`'s "Phase 35.7" section for the full reasoning. Committed on `feature/phase-35.7-dx12-sdf-rect` as `5376a89`, merged into `develop` as `ec0d7f8` (clean, no conflicts).

**Remaining Phase 35 work, roughly in order (not yet broken into named sub-phases beyond 35.7):**
- WebGPU: its own "first SDF rect" sub-phase next, likely reusing much of Phase 35.1/35.7's design (SPIR-V/DXIL→WGSL translation choice TBD — Dawn accepts WGSL directly, so this may mean hand-authoring a third shader source rather than a cross-compile step) — and, per Phase 35.2's own finding (reconfirmed independently by 35.7), deliberately test off-center, non-full-coverage geometry from this sub-phase's own first commit too.
- Then DX12/WebGPU's own `BatchKind::Image`/`Shadow`/`Layer`/`BackdropBlur` parity work, mirroring Vulkan's own Phase 35.2-35.6 sequence — not yet started for either backend.

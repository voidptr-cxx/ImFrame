...
**Discovered, not fixed (out of scope for 35.8, flagged separately):** running this sub-phase's own "full Debug build must be clean" verification bar as an actual non-targeted full build (rather than building only the WebGPU-specific targets) surfaced a pre-existing, unrelated compile failure in `Tests/Backends/DX12Conformance_test.cpp` — `std::numeric_limits<float>::max()` mangled by `<windows.h>`'s own `max` macro, surfacing inside the public `include/ImFrame/Tree/Element.hpp`/`Tree/Primitives/Box.hpp` headers, almost certainly a missing `NOMINMAX` somewhere in that test's own DX12-backend include chain. Lives entirely in Phase 22-era code, unrelated to WebGPU rendering — flagged as a background follow-up task rather than fixed here. `ctest` excluding just that one (already-unbuilt) target still ran 556/556 clean. **Still unfixed as of Phase 35.10** (flagged as background task `task_f18fb109`).

**Next Action:** Phase 35.9 (complete, this session) is DX12's own `BatchKind::Image` sub-phase, mirroring `NativeRendererVulkan`'s Phase 35.2 scope exactly: `Rendering::TextureId::Value()` reinterpreted as a raw `ID3D12Resource*` (already in `D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE`), matching `NativeRendererVulkan`'s own raw-`VkImageView` convention. `Shaders/Image.hlsl` binds its texture as a real `Texture2D` SRV (descriptor table, t0) plus a fixed **static sampler** (s0, declared directly in the root signature) — HLSL has no combined-image-sampler type, so this is the closest D3D12 analogue to Vulkan's one shared `VkSampler`. A new shader-visible `CBV_SRV_UAV` heap (`_srvHeap`) is grown on demand and fully written (one SRV per `Image` batch) *before* any command-list recording begins — the same "write everything before recording, since descriptor content resolves at execution time, not recording time" timing `NativeRendererVulkan::Render()`'s own per-batch descriptor-set write already established, avoiding the same "last write wins" race. `RenderRectBatch()`/`RenderImageBatch()` now each unconditionally rebind their own PSO/root-signature/CBV, since Rect and Image batches can now interleave within one `Render()` call. **Found and fixed a second pre-existing, unrelated bug** in `ViewportDX12.cpp` — Phase 35.7's own `Allocate()`-bool-into-`IMF_ASSERT` fix compiles fine in Debug but fails **Release** (`C4189` unused-variable, since `IMF_ASSERT` elides its condition entirely under `NDEBUG`) — fixed with `[[maybe_unused]]` on both locals (RTV and SRV slots). **Landed clean on the first real attempt** for all the actual Image-rendering logic — both new tests (single textured+tinted image, three-batch multi-texture race test) passed immediately once the `ViewportDX12.cpp` Release fix unblocked the Release build. Full suite **558/558** (excluding `ThreadPool stress` and the still-unfixed `DX12Conformance` `NOMINMAX` bug), `check_api_leaks.sh` clean, both Debug and Release rebuilt and passing. See `.claude/DECISIONS.md`'s "Phase 35.9" section for full reasoning. Committed on `feature/phase-35.9-dx12-image` as `d418f93`, merged into `develop` as `1cff8c2` (clean, no conflicts).

**Next Action:** Phase 35.10 (complete, this session) is WebGPU's own `BatchKind::Image` sub-phase, mirroring `NativeRendererVulkan`'s Phase 35.2 scope: `Rendering::TextureId::Value()` reinterpreted as a raw `WGPUTextureView`, matching Vulkan's own raw-`VkImageView` convention (not DX12's raw-`ID3D12Resource*` one). The inline WGSL Image shader splits texture/sampler into three separate bind-group entries (uniform b0 vertex, `texture_2d<f32>` b1 fragment, `sampler` b2 fragment) — closer to HLSL's SRV+sampler split (Phase 35.9) than GLSL's combined `sampler2D`. **A genuine simplification unique to this backend:** a `WGPUBindGroup` is immutable once created, so `RenderImageBatch()` just creates a fresh one per `Image` batch inline during the recording loop, uses it, and releases it — no descriptor-pool/heap capacity bookkeeping (`EnsureImageDescriptorCapacity()`-equivalent) exists on this backend at all, unlike Vulkan's/DX12's own Image sub-phases, both of which must write every batch's descriptor before recording begins to avoid a "last write wins" race that simply doesn't exist for immutable WebGPU bind groups. Texture upload is also simpler — `wgpuQueueWriteTexture()` uploads directly with no manual staging buffer or layout/state transition. **Found and fixed one real bug**, caught via a temporary push/pop error-scope diagnostic (the same technique Phase 35.8 used): `WGPUSamplerDescriptor{}`'s zero-initialized `maxAnisotropy` is `0`, which Dawn's validator rejects outright ("Max anisotropy (0.000000) is less than 1") — fixed with `samplerDesc.maxAnisotropy = 1;` (meaning "off"). **Landed clean immediately after that one fix** — both new Image tests (textured+tinted image, three-batch multi-texture test) passed. Full suite **560/560** (excluding `ThreadPool stress` and the still-open `DX12Conformance` `NOMINMAX` bug), `check_api_leaks.sh` clean, both Debug and Release built and passing (21 `[webgpu]`-labelled tests, up from 19). See `.claude/DECISIONS.md`'s "Phase 35.10" section for full reasoning. Committed on `feature/phase-35.10-webgpu-image` as `0c3979a`, merged into `develop` as `41b935e` (clean, no conflicts).

**This completes `BatchKind::Rect` + `BatchKind::Image` parity across all three of Vulkan/DX12/WebGPU.**

**Next Action:** Phase 35.11 (complete, this session) is DX12's own compute-shader Gaussian blur sub-phase — a new `BlurPassDX12` (`Backends/SDL3DX12/`), mirroring `BlurPassVulkan`'s Phase 35.3 scope and structure exactly (two `Dispatch()` ping-pong passes against `Shaders/GaussianBlur.hlsl`, lazy init, reallocate-targets-only-on-size-change). `BlurResult` is just `{ ID3D12Resource* Resource; }` — D3D12 has no persistent "image view" object the way Vulkan's `VkImageView` is, matching `NativeRendererDX12`'s own established raw-`ID3D12Resource*` convention (Phase 35.9). Both `uSource`/`uDest` are `RWTexture2D` UAVs (mirroring `GaussianBlur.glsl`'s own storage-image-for-both-bindings choice, not an SRV+UAV split). Direction/Radius are passed via `SetComputeRoot32BitConstants()` (root constants), the direct D3D12 analogue of Vulkan's push constants. **A genuine DX12-specific simplification:** the ping-pong targets are created directly in, and never leave, `D3D12_RESOURCE_STATE_UNORDERED_ACCESS` — no `_targetsNeedInitialTransition`-style bookkeeping is needed at all, since `CreateCommittedResource()` accepts an explicit initial state directly (unlike Vulkan, where every image starts in `UNDEFINED` and needs at least one transition). **Documented, not yet implemented:** unlike Vulkan's `GENERAL` layout (valid for both a render-target and storage-image role simultaneously), D3D12 has no single state serving both `RENDER_TARGET` and `UNORDERED_ACCESS` — whichever future sub-phase wires this into `RenderShadowBatch()` will need an explicit `RENDER_TARGET -> UNORDERED_ACCESS -> PIXEL_SHADER_RESOURCE` transition sequence around the silhouette target that Vulkan's own Phase 35.4 didn't need. **Found and fixed a third instance of this project's recurring `<windows.h>` min/max macro-pollution bug** — this one self-caused, in `BlurPassDX12.cpp`'s own `std::min(radius, 64.0f)` call — fixed immediately (in scope, since it's this sub-phase's own new code) by adding `#define NOMINMAX` before `#include <windows.h>` in `BlurPassDX12.hpp`; the other two instances (`DX12Conformance_test.cpp`, still open; `ViewportDX12.cpp`, fixed in Phase 35.9) remain as before. **Landed clean immediately after that fix** — all 3 new tests (no-op below radius 0, impulse-spread, ping-pong reuse) passing on the first real attempt. Full suite **563/563** (excluding `ThreadPool stress` and the still-open `DX12Conformance` bug), `check_api_leaks.sh` clean, both Debug and Release built and passing. See `.claude/DECISIONS.md`'s "Phase 35.11" section for full reasoning. Committed on `feature/phase-35.11-dx12-gaussian-blur` as `d877bd3`, merged into `develop` as `d8d0c54` (clean, no conflicts).

**Next Action:** Phase 35.12 (complete, this session) is DX12's own `BatchKind::Shadow` sub-phase, wiring `BlurPassDX12` (Phase 35.11) into `RenderShadowBatch()`, mirroring `NativeRendererVulkan`'s Phase 35.4 structure exactly (silhouette render → blur → composite draw). **Implements the resource-state transition sequence Phase 35.11's own comment flagged in advance:** the silhouette resource is created with both `D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET` and `_ALLOW_UNORDERED_ACCESS`, sitting "at rest" in `UNORDERED_ACCESS` between calls and transitioning to `RENDER_TARGET` only for the brief silhouette-draw window each call — no single D3D12 state serves both roles the way Vulkan's `GENERAL` layout does. The blurred result (borrowed from `BlurPassDX12`, which expects its own targets to stay in `UNORDERED_ACCESS`) is transitioned to `PIXEL_SHADER_RESOURCE` for the composite draw's SRV, then explicitly transitioned back afterward to restore that invariant. **A genuine D3D12-specific descriptor-heap constraint handled:** the composite draw's SRV shares `_srvHeap` (the same heap `BatchKind::Image` batches use, Phase 35.9) at one extra reserved slot, rather than a second heap — D3D12 only allows one `CBV_SRV_UAV` heap bound via `SetDescriptorHeaps()` at a time per command list, unlike Vulkan's unlimited simultaneously-bound descriptor sets; the slot is written *late* (after the blur completes, right before the composite draw), safe because an explicit fence wait already separates that write from any earlier command list's execution. `Render()`'s own command-list handling was refactored into reusable `BeginMainCommandList()`/`EndAndSubmitMainCommandList()` halves, mirroring Vulkan's own identical Phase 35.4 refactor (needed since `_blurPass.Apply()` is its own separate submission). `BuildImageQuadVertices()`'s own UV table stays **unflipped** (like Vulkan's, not GL3's) even though `Image.hlsl`'s vertex shader negates Y like GL's — the UV-flip and vertex-Y-negation questions are independent, and this backend's texture row order is top-down like Vulkan's, not bottom-up like GL's (already confirmed by every passing Image test). **Landed clean on the first real attempt** — all 6 tests (3 Rect/2 Image baseline + the new Shadow test) passing immediately despite the significant resource-transition bookkeeping involved. Full suite **564/564** (excluding `ThreadPool stress` and the still-open `DX12Conformance` bug), `check_api_leaks.sh` clean, both Debug and Release built and passing. See `.claude/DECISIONS.md`'s "Phase 35.12" section for full reasoning. Committed on `feature/phase-35.12-dx12-shadow` as `b5c75a9`, merged into `develop` as `b10b1c4` (clean, no conflicts).

**Next Action:** Phase 35.13 (complete, this session) is DX12's own `BatchKind::Layer` sub-phase, wiring `PushOpacityLayer`/`PushBlendLayer`/`PopLayer()` support into `NativeRendererDX12`, mirroring `NativeRendererVulkan::PushLayer()`/`PopLayer()`'s own Phase 35.5 structure. Each `LayerTarget` (one per stack depth, grown on demand) needs only `D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET` — simpler than Phase 35.12's own three-role shadow silhouette resource, since a layer target is never a compute UAV — sitting at rest in `PIXEL_SHADER_RESOURCE` and round-tripping through `RENDER_TARGET` only while pushed. `CompositeOpacityLayer()` reuses `Image.hlsl`/`_imageRootSignature` via a new `_premultipliedImagePipelineState` (`ONE`/`INV_SRC_ALPHA` blend factors, for the already-premultiplied captured layer scaled by `Opacity`). `CompositeBlendLayer()` needed a genuinely new shader, `Shaders/Blend.hlsl` (ported from `Blend.glsl`, ps_6_0/vs_6_0, no vertex buffer — a fixed fullscreen quad generated from `SV_VertexID`), and its own root signature/PSO. Both composite draws' SRVs share `_srvHeap` (Phase 35.9/35.12's own heap) at 3 newly-reserved slots (`imageBatchCount+0/+1/+2`) rather than a second heap — D3D12 still allows only one bound `CBV_SRV_UAV` heap per command list. `CopyBackdropForBlend()` (D3D12's analogue of Vulkan's `vkCmdCopyImage`-based backdrop capture) is called at the same point in `PopLayer()` Vulkan's own version is, even though D3D12 has no "active rendering instance" restriction forcing that ordering — kept for structural parity, documented as a choice not a requirement. **Found and fixed one real, self-caused bug**: `Blend.hlsl`'s fullscreen-quad UV table was ported unchanged from `Blend.glsl`, but Vulkan's Y-down NDC (where position `(-1,-1)` is genuinely screen-top-left) doesn't carry over to D3D12's Y-up NDC (where that same position is screen-*bottom*-left) — causing the `PushBlendLayer` test to sample a vertically mirrored, empty row of the popped layer. Fixed by flipping every UV table entry's V component, since this fixed-function shader has no per-frame transform to apply the correction to the way `SDFRect.hlsl`'s vertex shader does. **Landed clean immediately after that fix** — both new tests (`PushOpacityLayer` at half opacity, `PushBlendLayer` with `Multiply`) passing, all 8 `NativeRendererDX12` tests green. Full suite **565/565** (excluding `ThreadPool stress` and the still-open `DX12Conformance` bug), `check_api_leaks.sh` clean, both Debug and Release built and passing. See `.claude/DECISIONS.md`'s "Phase 35.13" section for full reasoning. Committed on `feature/phase-35.13-dx12-layer` as `dbedfa2`, merged into `develop` as `e6fcb6b` (clean, no conflicts).

**Next Action:** Phase 35.14 (complete, this session) is DX12's own `BatchKind::BackdropBlur` sub-phase, wiring `_blurPass` into `RenderBackdropBlurBatch()`, mirroring `NativeRendererVulkan::RenderBackdropBlurBatch()`'s own Phase 35.6 structure exactly: copy the padded requested region out of `_targetResource` via `CopyTextureRegion()`'s own `pSrcBox` parameter (no GL-style window-coordinate flip needed — D3D12/Vulkan both address texture memory top-down), blur it via `_blurPass.Apply()`, composite back a UV-cropped sub-rectangle at exactly the requested rect using the standard (non-premultiplied) `_imagePipelineState`. `_backdropBlurCopyResource` needs only `D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS` (never a render target — simpler than both Shadow's dual-role resource and Layer's RT-only one, since this resource is only ever a copy destination or `_blurPass`'s own UAV source/dest), sitting at rest in `UNORDERED_ACCESS` like `BlurPassDX12`'s own ping-pong targets. The composite draw reuses `_compositeSrvCpuBase` slot +0 (the same single-texture slot Shadow/`CompositeOpacityLayer()` already share) rather than a fourth dedicated slot. `BuildImageQuadVertices()` gained optional `uvMin`/`uvMax`/`radii` parameters (defaulting to the existing behavior, matching Vulkan's own full signature) so the composite draw can crop to exactly the requested rect out of the larger padded, blurred copy. **Landed clean on the first real attempt** — both new tests (colour-seam blend-and-crop, `MaxBackdropBlurPerFrame` rate-limit degradation) passing immediately, all 10 `NativeRendererDX12` tests green. Full suite **568/568** (excluding `ThreadPool stress` and the still-open `DX12Conformance` bug), `check_api_leaks.sh` clean, both Debug and Release built and passing. See `.claude/DECISIONS.md`'s "Phase 35.14" section for full reasoning. Committed on `feature/phase-35.14-dx12-backdropblur` as `f03bdef`, merged into `develop` as `bb856af` (clean, no conflicts).

**This completes DX12 parity with `NativeRendererVulkan`'s full Phase 35.1-35.6 milestone set** — `BatchKind::Rect`/`Image`/`Shadow`/`Layer`/`BackdropBlur` all implemented and tested for both Vulkan and DX12.

**Next Action:** Phase 35.15 (complete, this session) is WebGPU's own compute-shader Gaussian blur sub-phase — a new `BlurPassWebGPU` (`Backends/DawnWebGPU/`), mirroring `BlurPassVulkan`'s/`BlurPassDX12`'s own Phase 35.3/35.11 scope via an inline WGSL compute shader (compiled at runtime by Dawn, no offline pipeline, matching `NativeRendererWebGPU`'s own established convention). The source texture binds as a read-only `texture_storage_2d<rgba8unorm, read>`, the destination write-only — no `read_write` binding needed, since the math never writes in place. `BlurResult` ended up `{ WGPUTexture; WGPUTextureView }` (both handles, mirroring Vulkan's own pair) rather than "just a view," once it became clear a future `BackdropBlur` sub-phase (and this sub-phase's own test) needs the underlying texture for `CopyTextureToBuffer`/`CopyTextureToTexture`, which take no view. `Direction`/`Radius` need **two separate** uniform buffers (one per pass), not one rewritten between passes — `wgpuQueueWriteBuffer()` is itself a queued operation, and a second write enqueued before the consuming submit completes before that submit's own commands ever execute, a new (queue-ordering, not descriptor-content) instance of the "last write wins" hazard this codebase keeps encountering in different forms. Pass 1's bind group is created fresh per call (its own varying source), pass 2's reused (stable at a given size) — the natural idiom given `WGPUBindGroup`'s own immutability, with no Vulkan-style "mutate a descriptor set in place" equivalent to even attempt. **Found and fixed one real bug** via a temporary `ConsoleSink` attached to the Logger singleton (this project's `Logger` has zero sinks by default, so `IMF_ERROR`'s own uncaptured-WebGPU-error logging was being silently swallowed) — `CreateStorageTexture()` was missing `WGPUTextureUsage_CopySrc`, so reading back `Apply()`'s own returned result failed Dawn's copy-source validation, silently producing an invalid, no-op command buffer. **Landed clean immediately after that fix** — all 3 new tests (no-op below radius 0, impulse-spread, ping-pong reuse) passing. Full suite **571/571** (excluding `ThreadPool stress` and the still-open `DX12Conformance` bug), `check_api_leaks.sh` clean, both Debug and Release built and passing (24 `[webgpu]`-labelled tests, up from 21). See `.claude/DECISIONS.md`'s "Phase 35.15" section for full reasoning. Committed on `feature/phase-35.15-webgpu-gaussian-blur` as `4561c71`, merged into `develop` as `d843fe5` (clean, no conflicts).

**Next Action:** Phase 35.16 (complete, this session) is WebGPU's own `BatchKind::Shadow` sub-phase, wiring `BlurPassWebGPU` into `RenderShadowBatch()`, mirroring `NativeRendererVulkan`'s/`NativeRendererDX12`'s own Phase 35.4/35.12 structure (silhouette render → blur → composite). **Needs no resource-state transitions of any kind** — the silhouette texture is created with both `RenderAttachment` and `StorageBinding` usage and used directly in either role, generalizing `BlurPassWebGPU`'s own "WebGPU exposes no explicit resource-state model" finding to a render-attachment role too. `Render()`'s own single encoder/render-pass was refactored into reusable `BeginMainPass()`/`EndAndSubmitMainPass()` halves (mirroring Vulkan's/DX12's own `Begin`/`EndAndSubmit` split, needed since `_blurPass.Apply()` is its own separate submission) but needing **no fence wait** at the split — WebGPU's own sequential-submit-ordering guarantee already ensures correct execution order. `RenderRectBatch()` now (re)binds its own pipeline/bind-group unconditionally (matching `RenderImageBatch()`'s own convention), since a freshly-begun main pass after a Shadow batch starts with nothing bound. Added `BuildRectQuadVertices()`/`BuildImageQuadVertices()` helpers (new for this backend), keeping the same **unflipped** UV table Vulkan/DX12 use — WebGPU's own spec defines texture coordinate (0,0) as top-left unconditionally, regardless of the native backend Dawn targets underneath. **Landed clean on the first real attempt** — the new Shadow test passed immediately, all 6 `NativeRendererWebGPU` tests green. Full suite **572/572** (excluding `ThreadPool stress` and the still-open `DX12Conformance` bug), `check_api_leaks.sh` clean, both Debug and Release built and passing (25 `[webgpu]`-labelled tests, up from 24). See `.claude/DECISIONS.md`'s "Phase 35.16" section for full reasoning. Committed on `feature/phase-35.16-webgpu-shadow` as `8fa61dc`, merged into `develop` as `d283cc8` (clean, no conflicts).

**Next Action:** Phase 35.17 (complete) is WebGPU's `BatchKind::Layer` sub-phase (`PushOpacityLayer`/`PushBlendLayer`/`PopLayer`), mirroring Vulkan's/DX12's Phase 35.5/35.13. No resource-state transitions needed anywhere. **API change:** `NativeRendererWebGPU::SetTarget()` now takes `(WGPUTexture, WGPUTextureView, width, height)`, like Vulkan's/DX12's two-handle versions, because the blend layer's backdrop copy needs the real texture and WebGPU can't get it back from a view. **Differs from Vulkan/DX12:** each layer composite draw is bracketed by submissions (everything accumulated is submitted before the composite writes its shared buffers, and the composite is submitted right after). WebGPU has no push constants, so the blend mode and composite quads go through ordinary buffers read at execution time — two composites in one command buffer (e.g. two sibling layers, or a shadow followed by a layer pop) would otherwise both read the last write. Confirmed by a mutation test: the new shadow-plus-sibling-layers regression test fails under the naive design. **Vulkan and DX12 very likely have this same bug, untested** — flagged as follow-up task `task_b943d764`. The inline `kBlendWgsl` shader flips its fullscreen-quad V coordinates from the start (the lesson from Phase 35.13's `Blend.hlsl` bug). All 9 `NativeRendererWebGPU` tests pass; full suite **574/574** Debug and Release; `check_api_leaks.sh` clean. **Also found (not fixed), flagged as `task_f6372779`:** ctest silently dropped the `DX12Leak` test after the Release build (debug-only test + a discovery file shared across configs), and three `DX12Conformance` tests still run against a stale June binary — suite totals currently depend on which config was built last. See `.claude/DECISIONS.md` "Phase 35.17". Committed on `feature/phase-35.17-webgpu-layer` as `2a42f96`, merged into `develop` as `92b11ab` (clean, no conflicts).

**Next Action:** Phase 35.18 (complete) is WebGPU's `BatchKind::BackdropBlur`, mirroring Vulkan's/DX12's Phase 35.6/35.14: copy the padded region via `wgpuCommandEncoderCopyTextureToTexture()` (extent clamped, since an out-of-bounds copy invalidates the whole command buffer on WebGPU), blur via `_blurPass`, composite a UV-cropped quad. `BuildImageQuadVertices()` gained optional `uvMin`/`uvMax`/`radii` parameters. **Deliberate divergence:** it copies from and composites into the *current* target, so a backdrop blur inside a pushed layer blurs that layer's content (Vulkan/DX12 always read the real target). Covered by its own test. All 12 `NativeRendererWebGPU` tests pass; full suite **577/577** Debug and Release; `check_api_leaks.sh` clean. See `.claude/DECISIONS.md` "Phase 35.18". Committed on `feature/phase-35.18-webgpu-backdropblur` as `249fe10`, merged into `develop` as `610ba3a` (clean, no conflicts).

**WebGPU now has full `Rect`/`Image`/`Shadow`/`Layer`/`BackdropBlur` parity with Vulkan and DX12** — the breadth-first backend work chosen on 2026-09-13 is complete.

**Next Action:** Phase 35.19 (complete, user-requested fix of `task_b943d764`) fixes the Vulkan/DX12 internal-composite race. The ported WebGPU regression test failed on both *before* any fix — DX12 rendered layer 1 at layer 2's opacity; Vulkan's command buffer was invalidated outright by a descriptor set rewritten while bound. Fix: every internal quad draw now gets its own slot in `_internalQuadVertexBuffer`, and every composite its own SRV slot (DX12) or freshly allocated descriptor set (Vulkan), each written once per `Render()` call — no extra submissions, since those cost fence waits on these backends. The same test then exposed **two more Vulkan-only bugs**: missing pipeline barriers around layer targets in `PushLayer()`/`PopLayer()` (proven by mutation: 3/3 failures without, 10/10 passes with), and layer targets lacking `VK_IMAGE_USAGE_TRANSFER_SRC_BIT`, which made nested blend layers invalid usage (found as a validation error, while the pixel checks passed). Barriers were also added to `CopyBackdropForBlend()` per spec, though no test failed without those two. 8 new tests across the three backends; full suite **585/585** Debug and Release; zero Vulkan validation messages; `check_api_leaks.sh` clean. See `.claude/DECISIONS.md` "Phase 35.19". Committed on `fix/phase-35.19-layer-composite-buffer-race` as `b55aeea`, merged into `develop` as `865bdb3` (clean, no conflicts). **Test-harness gap noted:** Vulkan tests log validation errors but don't fail on them.

**Open follow-up tasks (flagged, not started):** `task_f6372779` — ctest silently drops the `DX12Leak` test depending on which config was built last, and runs three `DX12Conformance` tests against a stale June binary. `task_f18fb109` — the pre-existing `DX12Conformance` NOMINMAX compile error.

**Remaining Phase 35 work, roughly in order:**

...
**Next Action:** Phase 35.6 (complete, this session) added `DrawBackdropBlur` via a new `RenderBackdropBlurBatch()` — this closes out Phase 35's GL3-parity effects work (Shadow/Layer/BackdropBlur, Phases 35.4-35.6): `NativeRendererVulkan` now matches `NativeRendererGL3`'s full Phase 34 feature set. Follows Shadow's (35.4) heavier "flush, separate submissions, resume on a new command buffer" pattern, not Layer's (35.5) lighter one — `_blurPass.Apply()` is involved again, which is the real distinguishing factor (not "reads back existing content" in general, since `CompositeBlendLayer()` also does that yet stayed lightweight). A **new**, dedicated `_backdropBlurCopyImage` holds the padded copy region (deliberately separate from Phase 35.5's own `_backdropImage` — different size/lifecycle/usage-flags, mirroring `NativeRendererGL3`'s own identical choice to keep two textures). The region copy's offsets are used directly with **no coordinate flip** at all, unlike `NativeRendererGL3::RenderBackdropBlurBatch()`'s own GL-window-coordinate conversion — `vkCmdCopyImage` already addresses image space top-down, matching this codebase's own pixel-space convention. The composite draw reuses `_imagePipeline` (standard blend), not `_premultipliedImagePipeline` — matches `RenderShadowBatch()`'s own identical choice and GL3's own `RenderImageBatch()` reuse for this exact case. **Landed clean on the first real attempt** — zero validation-layer warnings for both the Y-asymmetric seam/crop test and the `MaxBackdropBlurPerFrame` rate-limit test (mirroring `NativeRendererGL3_test.cpp`'s own Phase 34.6 scenarios). Full suite **533/533** (531 baseline + 2 new tests), `check_api_leaks.sh` clean. See `.claude/DECISIONS.md`'s "Phase 35.6" section for the full reasoning. Committed on `feature/phase-35.6-vulkan-backdrop-blur` as `c708ce4`, merged into `develop` as `66b9a5a` (clean, no conflicts).

**Remaining Phase 35 work, roughly in order (not yet broken into named sub-phases beyond 35.6):**
- Vulkan: real integration into `SDL3VulkanBackend`'s live frame loop (replacing the current per-`Render()`-call CPU stall with proper semaphore sync) and `IRenderer` batching/text.

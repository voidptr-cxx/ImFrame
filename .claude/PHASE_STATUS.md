...
**Next Action:** Phase 35.5 (complete, this session) added layer compositing (`PushOpacityLayer`/`PushBlendLayer`/`PopLayer`) via `PushLayer()`/`PopLayer()`/`HandleLayerMarker()`. Unlike Shadow (Phase 35.4), no separate submission is needed — push/pop just end and re-begin rendering instances within the *same* still-recording main command buffer. `SetTarget()` gained a `VkImage targetImage` parameter (alongside the existing view) since blend-mode compositing's backdrop copy needs a real `vkCmdCopyImage` source and no Vulkan call recovers an image from a view — the same lesson `BlurResult` already established (Phase 35.3). Opacity compositing needed a **second** graphics pipeline (`_premultipliedImagePipeline` — same shader/layout as `_imagePipeline`, only `ONE`/`ONE_MINUS_SRC_ALPHA` blend factors, since Vulkan bakes blend state into the pipeline object). Blend-mode compositing ported `NativeRendererGL3`'s own 12-blend-mode shader to a new `Shaders/Blend.glsl` (vertex stage generates its fixed fullscreen quad from `gl_VertexIndex`, no vertex buffer at all), split into `CopyBackdropForBlend()` (the `vkCmdCopyImage`, illegal inside an active rendering instance) and `CompositeBlendLayer()` (the draw, needs one active) called from either side of `PopLayer()`'s own `vkCmdBeginRendering`. **Landed clean on the first real attempt** — zero validation-layer warnings for both the opacity and Multiply-blend tests (mirroring `NativeRendererGL3_test.cpp`'s own Phase 34.5 scenarios). Full suite **531/531** (529 baseline + 2 new tests), `check_api_leaks.sh` clean. See `.claude/DECISIONS.md`'s "Phase 35.5" section for the full reasoning. Committed on `feature/phase-35.5-vulkan-layer-compositing` as `c583ed7`, merged into `develop` as `69358c6` (clean, no conflicts).

**Remaining Phase 35 work, roughly in order (not yet broken into named sub-phases beyond 35.5):**
- Vulkan: `DrawBackdropBlur`, mirroring GL3's Phase 34.6 work (reuse `_blurPass`/the `GENERAL`-everywhere pattern already established — check first whether it can reuse `_compositeDescriptorSet` or needs its own), then real integration into `SDL3VulkanBackend`'s live frame loop (replacing the current per-`Render()`-call CPU stall with proper semaphore sync) and `IRenderer` batching/text.

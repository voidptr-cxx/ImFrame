...
**Open follow-up tasks (flagged, not started):** `task_b943d764` — Vulkan/DX12 Layer composites very likely share the buffer race fixed for WebGPU in Phase 35.17 (port the regression test first). `task_f6372779` — ctest silently drops the `DX12Leak` test depending on which config was built last, and runs three `DX12Conformance` tests against a stale June binary. `task_f18fb109` — the pre-existing `DX12Conformance` NOMINMAX compile error.

...
**Next Action:** Phase 35.3 (complete, this session) added `BlurPassVulkan` — a separable Gaussian blur via two `vkCmdDispatch` compute passes against a new `Shaders/GaussianBlur.glsl` (extended `cmake/CompileShader.cmake` to accept a `"comp"` stage for it), mirroring `BlurPassGL3`'s exact weight formula and single-program-both-directions design (direction/radius via a 12-byte push-constant block, not a UBO). `Apply()`'s return type is a new `BlurResult{VkImage; VkImageView}` struct rather than a bare view — no Vulkan call recovers an image from a view, and every real use (readback, barrier, copy) needs both handles. Ping-pong targets stay in `VK_IMAGE_LAYOUT_GENERAL` for their whole lifetime once allocated, with a real `vkCmdPipelineBarrier` memory barrier between the two dispatches (mandatory — `vkCmdDispatch` calls on the same queue have no implicit memory-visibility ordering). **Landed clean on the first real attempt** — zero validation-layer warnings in a Debug build, all 3 new tests (mirroring `BlurPassGL3_test.cpp`'s own no-op/impulse-spread/ping-pong-reuse scenarios) passing in both Debug and Release with no debugging cycle, unlike Phase 35.2. Full suite **528/528** (525 baseline + 3 new tests), `check_api_leaks.sh` clean. See `.claude/DECISIONS.md`'s "Phase 35.3" section for the full reasoning. Committed on `feature/phase-35.3-vulkan-gaussian-blur`, merged into `develop` — see this file's own merge-hash follow-up note below.

**Remaining Phase 35 work, roughly in order (not yet broken into named sub-phases beyond 35.3):**
- Vulkan: wire `BlurPassVulkan` into `DrawShadow`/layer-compositing/`DrawBackdropBlur` mirroring GL3's Phase 34.4–34.6 work (this is what actually exercises the blur pass from `NativeRendererVulkan::Render()` — 35.3 built and tested the pass standalone, not yet wired in), then real integration into `SDL3VulkanBackend`'s live frame loop (replacing the current per-`Render()`-call CPU stall with proper semaphore sync) and `IRenderer` batching/text.

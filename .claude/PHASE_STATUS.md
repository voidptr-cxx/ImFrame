...
**Next Action:** Phase 35.4 (complete, this session) wired `BlurPassVulkan` into `BatchKind::Shadow` via a new `RenderShadowBatch()` — silhouette render (opaque-white rounded rect into a new dual-purpose `_shadowSilhouetteImage`, `COLOR_ATTACHMENT_BIT | STORAGE_BIT`, kept in `VK_IMAGE_LAYOUT_GENERAL` forever), `_blurPass.Apply()`, then a tinted composite draw through a **second**, dedicated descriptor set (`_compositeDescriptorSet`) since `_blurPass`'s output never leaves `GENERAL` — unlike every real `DrawImage` texture, which `Render()`'s existing per-batch descriptor path hardcodes as `SHADER_READ_ONLY_OPTIMAL`. Required splitting `Render()`'s single "one command buffer, submit once" flow into reusable `BeginMainCommandBuffer()`/`EndAndSubmitMainCommandBuffer()` halves, since `_blurPass.Apply()` is its own fully separate, synchronously-awaited submission that can't be recorded into a different not-yet-submitted command buffer — `RenderShadowBatch()` flushes the accumulated command buffer, does its own silhouette-render + blur submissions, then begins a fresh one for the composite draw and everything after it. The composite quad's UV table is deliberately **unflipped** (unlike `NativeRendererGL3::BuildImageQuadVertices()`'s own V-flip) — this backend's vertex shader no longer negates Y (Phase 35.2) and a Vulkan image's row 0 is already its top row, so GL3's flip would have been a wrong port here, not a harmless one. **Two real bugs, both caught by the Debug validation layer on the first test run:** a barrier's `dstStageMask` didn't cover every stage its `dstAccessMask` needed (missing `COMPUTE_SHADER_BIT` for a later `imageLoad`), and `BlurPassVulkan`'s own ping-pong images lacked `VK_IMAGE_USAGE_SAMPLED_BIT` (needed now that a consumer binds them through a real sampler, not just `imageLoad`/`imageStore`). Full suite **529/529** (528 baseline + 1 new test — the same off-center shadow+rect scenario `NativeRendererGL3_test.cpp`'s own Phase 34.4 test uses), `check_api_leaks.sh` clean, both Debug (validation-clean after the fixes) and Release rebuilt and passing. See `.claude/DECISIONS.md`'s "Phase 35.4" section for the full reasoning. Committed on `feature/phase-35.4-vulkan-drawshadow` as `705ee00`, merged into `develop` as `81dd5c5` (clean, no conflicts).

**Remaining Phase 35 work, roughly in order (not yet broken into named sub-phases beyond 35.4):**
- Vulkan: layer compositing (`PushOpacityLayer`/`PushBlendLayer`/`PopLayer`) and `DrawBackdropBlur`, mirroring GL3's Phase 34.5/34.6 work (`RenderShadowBatch()`'s new `_compositeDescriptorSet`/`GENERAL`-layout pattern established this sub-phase is very likely reusable there too — check first before building a third variant), then real integration into `SDL3VulkanBackend`'s live frame loop (replacing the current per-`Render()`-call CPU stall with proper semaphore sync) and `IRenderer` batching/text.
- Then DX12 and WebGPU, each starting from their own "first SDF rect" sub-phase, likely reusing much of Phase 35.1's design (SPIR-V→DXIL/WGSL translation choices TBD per-backend) — and, per Phase 35.2's own finding, each backend's *first* sub-phase should deliberately test off-center, non-full-coverage geometry from the start rather than only symmetric shapes, to catch any analogous Y-axis-convention mismatch immediately instead of several sub-phases later.

...
TEST_CASE("NativeRendererDX12 keeps a Shadow composite and two sibling opacity layers independent "
...

    {
        ...
        // SRV slot that is rewritten before the command list executes, the earlier one reads the
        ...
        auto [pixels, rowPitch] = target.ReadPixels();
        const Pixel halfRed   = Sample(pixels, 25, 25, WIDTH, rowPitch);
        const Pixel fullBlue  = Sample(pixels, 65, 25, WIDTH, rowPitch);
        const Pixel shadow    = Sample(pixels, 91, 91, WIDTH, rowPitch);
        const Pixel untouched = Sample(pixels, 5, 120, WIDTH, rowPitch);
...
TEST_CASE("NativeRendererDX12 composites two sibling PushBlendLayers independently within one "
...

    {
        ...
        auto [pixels, rowPitch] = target.ReadPixels();
        const Pixel left         = Sample(pixels, 25, 25, WIDTH, rowPitch);
        const Pixel right        = Sample(pixels, 65, 25, WIDTH, rowPitch);
        const Pixel backdropOnly = Sample(pixels, 5, 90, WIDTH, rowPitch);
...
TEST_CASE("NativeRendererDX12 composites a PushBlendLayer nested inside an opacity layer against the "
...

    {
        ...
        auto [pixels, rowPitch] = target.ReadPixels();
        const Pixel inside  = Sample(pixels, 25, 25, WIDTH, rowPitch);
        const Pixel outside = Sample(pixels, 5, 90, WIDTH, rowPitch);

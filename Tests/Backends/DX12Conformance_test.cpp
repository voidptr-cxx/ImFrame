...
//
// NOTE: this backend is not built in this dev environment's CMake cache
// (IMF_BUILD_DX12_BACKEND=OFF) — written and reviewed following the exact
// same pattern already verified working on Vulkan/WebGPU/OpenGL3, but not
// locally compiled or run here, matching the existing precedent for the
// Metal backend (see .claude/DECISIONS.md row for Metal: "written and
// reviewed without ever compiling it").

TEST_CASE("SDL3DX12Backend renders the full ConformanceApp widget tree across all built-in themes", "[dx12]")
{
    SDL3DX12Backend backend(/*headless=*/true);
    REQUIRE(backend.Init(ConformanceWindowConfig()).has_value());
...
TEST_CASE("SDL3DX12Backend renders the full ConformanceApp widget tree across several layout sizes", "[dx12]")
{
